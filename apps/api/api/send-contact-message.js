...
function validatePayload(payload) {
  ...
  if (!CUSTOMER_TYPE_LABEL[payload.customerType]) return 'Invalid customer type.'
  return ''
}

function buildEmailHtml(payload) {
  const pairs = [
    ['Customer Type', CUSTOMER_TYPE_LABEL[payload.customerType]],
  ...
  return `
    ...
    <p>A new message was submitted from the website footer form.</p>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      subject: `[Website] New message from ${String(payload.companyName).trim()}`,
